...
if nargs == 1:
    ...
    #==================================================
    print("Q2(a): IMPLEMENTING PEGASOS ALGORITHM")
    k = 100             #BATCH SIZE
    W = np.matrix(np.zeros(X.shape[1],float))
    B = 0.0             #BIAS TERM
    numClasses = 10
    ...
    print("Q2(b): IMPLEMENTING ONE VS ONE MULTI CLASS SVM")
    CLASSIFIER_ARRAY,CLASSES_PAIR = oneVsOneClassification(X,Y,numClasses,k,epsilon)
    print("\n")

    print("Q2(b): STORING PEGASOS MODEL DATA TO FILE Q2_MODEL1")
    model1_file = open("Q2_MODEL1","wb")
    ...
    print("Q2(b): TRAINING DATA ACCURACY")
    accuracy = getAccuracy(X,Y,CLASSIFIER_ARRAY,CLASSES_PAIR,'')
    print("Q2(b): Training set accuracy = ",accuracy)
    print("\n")

    #==================================================

    print("Q2(b): TEST DATA ACCURACY")
    accuracy = getAccuracy(X_test,Y_test,CLASSIFIER_ARRAY,CLASSES_PAIR,'')
    print("Ques2(b): Test set accuracy = ",accuracy)
    print("\n")
    
    #==================================================
    
    print("Q2(c): IMPLEMENTING LIBSVM LINEAR KERNEL")
    linear_model = svm_train(list(Y.flat), X.tolist(), '-s 0 -t 0 -c 1')
    l,a,v = svm_predict(list(Y_test.flat), X_test.tolist(), linear_model)
    print("Q2(c): Linear kernel accuracy = ", a[0])
    print("\n")

    print("Q2(c): STORING LIBSVM LINEAR MODEL DATA TO FILE Q2_MODEL2")
    svm_save_model('Q2_MODEL2',linear_model)
    print("\n")
    
    print("Q2(c): IMPLEMENTING LIBSVM GAUSSIAN KERNEL")
    ...
    print("Q2(c): Gaussian kernel accuracy = ", a[0])
    print("\n")

    #==================================================

    print("Q2(d): IMPLEMENTING 10 FOLD CROSS VALIDATION USING LIBSVM GAUSSIAN KERNEL")

    cross_val_acc = []
    test_val_acc = []
    log_c_val = []

    print("Q2(d): LIBSVM CROSS VALIDATION FOR C = 0.00001")
    log_c_val.append(np.log(0.00001))
    accuracy = svm_train(list(Y.flat), X.tolist(), '-s 0 -t 2 -c 0.00001 -g 0.05 -v 10 -h 0')
    print("Q2(d): Cross Validation Accuracy = ", accuracy)
    cross_val_acc.append(accuracy)

    print("Q2(d): LIBSVM TEST ACCURACY FOR C = 0.00001")
    ...
    print("Q2(d): STORING BEST LIBSVM GAUSSIAN MODEL DATA FOR C = 10 TO FILE Q2_MODEL3")
    svm_save_model('Q2_MODEL3',gaussian_model6)
    print("\n")
    
    print("Q2(e): CONFUSION MATRIX FOR C = 10")
    confusionMat = drawConfusionMatrix(list(Y_test.flat),l,10)
    print("Q2(e): Confusion Matrix = ", confusionMat)
    print("\n")    
    
    print("Q2(d): PLOTTING CROSS VALIDATION ACCURACY AND TEST ACCURACY VS C")
